...
class pantalla_merienda : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val btnRegreMerienda = findViewById<Button>(R.id.btnRegreMerienda)

        btnRegreMerienda.setOnClickListener{

            val intent = Intent(this, pantalla_principal::class.java)
            startActivity(intent)
        }
        val btnEliminarMerienda = findViewById<Button>(R.id.btnEliminarMerienda)

        btnEliminarMerienda.setOnClickListener{
            val sharedPreferences = this.getSharedPreferences("CaloriasMerienda", Context.MODE_PRIVATE)
            val editor = sharedPreferences.edit()
            editor.clear()
            editor.apply()

            val sharedPreferences2 = this.getSharedPreferences("AlimentosMerienda", Context.MODE_PRIVATE)
